...
1. Wizard "Nova Apresentação" em 3 passos (dados → estrutura → tema).
2. Route handler `/api/generate` + Claude com structured output no schema da Vizu (validação Zod).
3. Biblioteca de layouts semânticos (capa, agenda, seção, bullets, KPI, comparação, tabela, encerramento).
4. Botão "Regenerar slide" e "Melhorar texto" por slide.
...
- **Agent D — "ai-generation"**: Fase 4 (wizard + /api/generate + layouts semânticos).
